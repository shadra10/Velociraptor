...
public class UI : MonoBehaviour
{
    ...
    public List<GameObject> playerEggs;
    public GameObject EggParent, hoverUsed;
    ...
    void OnGUI()
    {
        ...
        for (int i = 0; i < units.Count; i++)
        {
            ...
            {
                for (int y = 0; y < 3 && current.localPosition.y + y <= height+100; y++)
                {
                    texture.SetPixel((int)current.localPosition.x + x+offset, (int)current.localPosition.y + y+offset, Color.blue);
                }
            }
        }
        // for (unitX.)
        ...
        GUI.DrawTexture(new Rect(20, 490, 200, 90), texture, ScaleMode.StretchToFill, true, 10.0F);
